...
# 获取第一个 session ID
session_id=$(echo "$sessions_json" | node -e "
const data = JSON.parse(require('fs').readFileSync(0));
if (Array.isArray(data) && data.length > 0) {
    console.log(data[0].id || data[0].sessionKey || '');
} else {
    console.log('');
}
" 2>/dev/null || echo "")
...
# 获取 session 历史（最多 10 条）
history=$($OPENCLAW_CMD sessions history --sessionKey "$session_id" --json 2>/dev/null | head -100 || echo "[]")
