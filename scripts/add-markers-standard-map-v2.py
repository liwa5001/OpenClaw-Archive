#!/usr/bin/env python3
"""
在标准 Mercator 投影地图上添加精确的城市标记 v2
修复标记点不显示的问题
"""

import matplotlib.pyplot as plt
import matplotlib.patches as patches
import matplotlib.image as mpimg
import math

# 7 个城市的精确坐标 (纬度，经度)
CITIES = {
    "Suzhou": (31.2990, 120.5853),      # 中国苏州
    "Bangalore": (12.9716, 77.5946),    # 印度班加罗尔
    "Queretaro": (20.5888, -100.3899),  # 墨西哥克雷塔罗
    "Garching": (48.2487, 11.6519),     # 德国加兴
    "Novi": (42.4806, -83.4755),        # 美国诺维
    "Lodz": (51.7592, 19.4560),         # 波兰罗兹
    "Nagoya": (35.1815, 136.9066),      # 日本名古屋
}

# 读取标准地图背景
bg_image_path = '/Users/liwang/.openclaw/workspace/standard-mercator-map-bg-v2.png'
img = mpimg.imread(bg_image_path)
img_height, img_width = img.shape[:2]

print(f"📐 地图尺寸：{img_width} x {img_height}")

# 创建图形 (使用像素坐标系)
fig, ax = plt.subplots(figsize=(16, 9), dpi=150)
ax.imshow(img)

# Mercator 投影转换：经纬度 -> 像素坐标
def mercator_to_pixel(lon, lat, map_width, map_height):
    """
    将经纬度转换为像素坐标 (Mercator 投影)
    假设地图是标准 Mercator，覆盖 -180~180 经度，-85~85 纬度
    """
    # 经度 -> x 像素 (线性映射)
    x = (lon + 180) / 360 * map_width
    
    # 纬度 -> y 像素 (Mercator 投影)
    lat_rad = math.radians(lat)
    merc_y = math.log(math.tan(math.pi/4 + lat_rad/2))
    max_merc_y = math.log(math.tan(math.pi/4 + math.radians(85)/2))
    
    # Mercator Y 归一化并翻转为图像坐标 (y 轴向下)
    y_normalized = merc_y / max_merc_y
    y = (1 - y_normalized) / 2 * map_height
    
    return x, y

# 城市标记样式
MARKER_COLOR = "#2E86DE"  # 鲜明的商务蓝色
MARKER_RADIUS = 12  # 稍大一点

# 标签偏移
LABEL_OFFSETS = {
    "Garching": (-25, -20),
    "Lodz": (20, 15),
    "Novi": (20, -20),
    "Bangalore": (20, -20),
    "Suzhou": (20, -20),
    "Nagoya": (20, -20),
    "Queretaro": (20, -20),
}

# 绘制城市标记
for city, (lat, lon) in CITIES.items():
    x, y = mercator_to_pixel(lon, lat, img_width, img_height)
    
    print(f"📍 {city}: ({lat:.2f}°N, {lon:.2f}°E) -> 像素 ({x:.1f}, {y:.1f})")
    
    # 外圈光晕 (最大，最淡)
    glow = patches.Circle((x, y), MARKER_RADIUS + 8, 
                          color=MARKER_COLOR, alpha=0.25, zorder=10)
    ax.add_patch(glow)
    
    # 阴影 (右下偏移)
    shadow = patches.Circle((x - 2, y + 2), MARKER_RADIUS, 
                            color='#888888', alpha=0.5, zorder=20)
    ax.add_patch(shadow)
    
    # 蓝色标记点 (实心)
    marker = patches.Circle((x, y), MARKER_RADIUS, 
                            color=MARKER_COLOR, alpha=1.0, zorder=30)
    ax.add_patch(marker)
    
    # 白色内圈 (让标记更醒目)
    inner = patches.Circle((x, y), MARKER_RADIUS - 4, 
                           color='white', alpha=0.3, zorder=31)
    ax.add_patch(inner)
    
    # 标签偏移
    offset_x, offset_y = LABEL_OFFSETS.get(city, (20, -20))
    
    # 城市名称标签 (加大字体和背景)
    ax.annotate(city, xy=(x, y), xytext=(x + offset_x, y + offset_y),
                textcoords='data',
                fontsize=12, color='#222222', weight='bold',
                bbox=dict(boxstyle='round,pad=0.6', facecolor='white', 
                         edgecolor='#DDDDDD', linewidth=1, alpha=1.0),
                zorder=40)

# 移除坐标轴
ax.axis('off')
ax.set_xlim(0, img_width)
ax.set_ylim(img_height, 0)  # 图像坐标 y 轴向下

plt.tight_layout(pad=0)

# 保存文件 (高分辨率)
output_path = '/Users/liwang/.openclaw/workspace/world-map-standard-final-v2.png'
plt.savefig(output_path, dpi=200, facecolor='#FFFFFF', edgecolor='none',
            bbox_inches='tight', pad_inches=0.02)

print(f"\n✅ 地图生成完成！")
print(f"📂 文件位置：{output_path}")
