#!/usr/bin/env python3
"""
创建 AMP HC 汇总 Sheet v2
- Juarez 归为 Queretaro
- Farmington Hills 改为 Novi
"""
...
for loc in all_locations:
    if loc == 'Unknown' or pd.isna(loc) or loc == 'nan' or loc == '0':
        continue
...
for _, row in summary_df.iterrows():
    site = row['Site']
    map_data[site] = {
        'Internal_2025': int(row['Internal_2025']),
        'External_2025': int(row['External_2025']),
        'Total_2025': int(row['Total_2025']),
        'Internal_2026': int(row['Internal_2026']),
        'External_2026': int(row['External_2026']),
        'Total_2026': int(row['Total_2026']),
    }

json_path = '/Users/liwang/.openclaw/workspace/Temp/amp-hc-data-v2.json'
